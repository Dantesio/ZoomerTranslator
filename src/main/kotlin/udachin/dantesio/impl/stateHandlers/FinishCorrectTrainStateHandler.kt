...
val FINISH_CORRECT_TRAIN_STATE: State = State("FINISH_CORRECT_TRAIN_STATE")

class FinishCorrectTrainStateHandler(override val state: State = FINISH_CORRECT_TRAIN_STATE) : StateHandler {
    override fun bump(bot: TelegramBot, chat: Chat, state: State, update: Update) {
        ...
        message.text = "Отлично! Вы совершенно правы!"
        bot.execute(message)
